...
def plot_tsne(reduced_features, labels, title="t-SNE Visualization", verbose=True, savefig=False):
    """
    ...
    scatter = plt.scatter(reduced_features[:, 0], reduced_features[:, 1], c=labels, cmap='tab20', alpha=0.7)
    if verbose:
...
def plot_label_timeline(labels, label_names=None, cmap='tab20', title='Label Timeline', show_axes=True):
    """
    Plots a timeline with colors corresponding to each label index in the input vector.

    Args:
        labels (list or np.array): Vector of labels (integers).
        label_names (list, optional): Names corresponding to each label index.
        cmap (str or Colormap, optional): Matplotlib colormap to use. Default is 'tab10'.
        title (str, optional): Title of the plot.
        show_axes (bool, optional): Whether to show axes, legend, and title.
    """
    ...
    # Create segments where the label stays the same
    segments = []
    start = 0
    ...
    fig, ax = plt.subplots(figsize=(10, 1.5))  # Smaller height for a compact timeline

    # Draw the segments
    for start, end, label in segments:
        ax.axvspan(start, end, color=cmap(norm(label)),
                   label=label if label_names is None else label_names[label])
